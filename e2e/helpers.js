...
export async function mockApi(page) {
  ...
  await page.route('**/api/**', (route) => {
    ...
    if (method === 'GET' && path === '/api/settings') {
      return ok({
        settings: { defaultInactivityDays: DEFAULT_INACTIVITY, syncIntervalMinutes: 60, githubOwners: '' },
        defaults: { defaultInactivityDays: DEFAULT_INACTIVITY, syncIntervalMinutes: 60, githubOwners: '' },
      });
    }
