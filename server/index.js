...
// ---- gh CLI quick actions -------------------------------------------------
// All three endpoints validate the repo against repoCache so user-supplied
// :id never reaches a shell. full_name comes from our own cache, never from
// request params or body — no shell injection is possible.

app.post('/api/repos/:id/gh/open', (req, res) => {
  const repo = findRepo(Number(req.params.id));
  if (!repo) return res.status(404).json({ error: 'repo not found' });
  try {
    execFileSync('gh', ['repo', 'view', repo.full_name, '--web'], { stdio: 'ignore' });
    res.json({ ok: true });
  } catch (e) {
...
app.get('/api/repos/:id/gh/prs', (req, res) => {
  const repo = findRepo(Number(req.params.id));
  if (!repo) return res.status(404).json({ error: 'repo not found' });
  try {
    const out = execFileSync(
      'gh', ['pr', 'list', '--repo', repo.full_name, '--state', 'open',
...
app.post('/api/repos/:id/gh/issue', (req, res) => {
  const repo = findRepo(Number(req.params.id));
  if (!repo) return res.status(404).json({ error: 'repo not found' });
  ...
  try {
    const out = execFileSync(
      'gh', ['issue', 'create', '--repo', repo.full_name, '--title', title,
             '--body', body || ' '],
      { encoding: 'utf8' }
    ).trim();
