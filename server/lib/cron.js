...
function parseField(field) {
  ...
  if (field.startsWith('*/')) {
    ...
    if (!Number.isFinite(step) || step < 1) throw new Error(`invalid cron step "${field}"`);
    return (v) => v % step === 0;
  }
...
export function parseCron(expr) {
  ...
  if (parts.length !== 5) throw new Error(`cron expression must have exactly 5 fields: "${expr}"`);
