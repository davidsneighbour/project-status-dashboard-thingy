...
const repoA = {
  ...
  ignored: false,
  notice_count: 2,
  latest_notice: { body: 'card preview note', created_at: '2026-06-02T00:00:00.000Z' },
};
...
describe('ignore flag + notices UI', () => {
  beforeEach(() => {
    ...
    api.allNotices.mockResolvedValue({
      notices: [
        ...
        { id: 12, repo_id: 2, full_name: 'user/repo-b', body: 'beta note', created_at: '2026-06-02T00:00:00.000Z' },
      ],
    });
  });
  ...
  it('hides ignored repos until the show-ignored toggle is on', async () => {
    ...
    expect(await screen.findByRole('link', { name: 'repo-b' })).toBeInTheDocument();
    expect(screen.getByText('ignored')).toBeInTheDocument();
    expect(window.localStorage.getItem('repo-triage-show-ignored')).toBe('true');
  });
  ...
  it('wires the card menu ignore + add-notice actions to the API', async () => {
    ...
    fireEvent.change(screen.getByRole('textbox', { name: 'New notice' }), { target: { value: 'a fresh note' } });
    fireEvent.click(screen.getByRole('button', { name: 'Add' }));
    await waitFor(() => expect(api.addNotice).toHaveBeenCalledWith(1, 'a fresh note'));
