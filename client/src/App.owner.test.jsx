...
const card = (over) => ({
  id: 1, name: 'r', full_name: 'o/r', owner: 'o', owner_type: 'Organization',
  html_url: 'https://x/r', description: 'd', private: false, archived: false, fork: false,
  language: 'JS', pushed_at: '2026-06-01T00:00:00.000Z', checkedAgeDays: 0, dueInDays: 7,
...
describe('owner indicator', () => {
  ...
  it('shows an owner badge per card when the board mixes owners', async () => {
    api.list.mockResolvedValue({
      ...
      repos: [
        card({ id: 1, name: 'a', full_name: 'davidsneighbour/a', owner: 'davidsneighbour' }),
    ...
    expect(await screen.findByTitle('owner: davidsneighbour')).toBeInTheDocument();
  ...
  it('hides the owner badge for a single-owner board', async () => {
    api.list.mockResolvedValue({
      ...
      repos: [card({ id: 1, name: 'a', full_name: 'davidsneighbour/a', owner: 'davidsneighbour' })],
    });
  ...
  it('renders a source-warning banner from the API', async () => {
    api.list.mockResolvedValue({
      ...
      repos: [card({ id: 1, full_name: 'gohugo-ananke/x', owner: 'gohugo-ananke' })],
      sourceWarnings: ['Token is not a member of organization "gohugo-ananke" — loaded its public repositories only.'],
    });
