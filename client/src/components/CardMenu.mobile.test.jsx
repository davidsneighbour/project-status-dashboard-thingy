...
const repo = { id: 1, name: 'widget', tags: ['infra'], priority: null, notice_count: 0 };

const noop = () => {};
const handlers = {
  onSetChecked: noop, onClearCheck: noop, onSetPriority: noop, onSetInactivity: noop,
  onSetIgnored: noop, onAddNotice: noop, onViewNotices: noop, onAddTag: noop, onRemoveTag: noop,
  onClose: noop, defaultInactivity: 7,
};
...
describe('CardMenu bottom-sheet variant (mobile)', () => {
  ...
  it('exposes the same action groups as desktop', () => {
    setMobileViewport();
    render(<CardMenu repo={repo} anchorRef={{ current: null }} {...handlers} />);
    ...
    expect(screen.getByRole('button', { name: 'Checked now' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Move to Today' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Clear check date' })).toBeInTheDocument();
    expect(screen.getByRole('group', { name: 'Set triage priority' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Save' })).toBeInTheDocument();
    expect(screen.getByLabelText('New tag')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Ignore repo' })).toBeInTheDocument();
  ...
  it('keeps the anchored popover layout on desktop', () => {
    ...
    render(<CardMenu repo={repo} anchorRef={{ current: anchor }} {...handlers} />);
    const dialog = screen.getByRole('dialog', { name: 'Settings for widget' });
