...
describe('MoveSheet', () => {
  it('is a bottom sheet titled with the repo name and a single numeric field', () => {
    render(<MoveSheet repo={repo} defaultInactivity={7} onApply={() => {}} onClose={() => {}} />);
    const dialog = screen.getByRole('dialog', { name: 'Reschedule widget' });
    expect(dialog).toHaveTextContent('widget');
    expect(screen.getByLabelText('Mark done for (days)')).toHaveAttribute('type', 'number');
  });

  it('defaults the field to the review cycle', () => {
    render(<MoveSheet repo={repo} defaultInactivity={14} onApply={() => {}} onClose={() => {}} />);
  ...
  it('applies the entered number and closes on "Mark done"', () => {
    const onApply = vi.fn();
    const onClose = vi.fn();
    render(<MoveSheet repo={repo} defaultInactivity={7} onApply={onApply} onClose={onClose} />);

    fireEvent.change(screen.getByLabelText('Mark done for (days)'), { target: { value: '21' } });
