...
const repo = {
  id: 1, name: 'widget', html_url: 'https://x/widget', description: 'a thing',
  private: false, archived: false, fork: false, language: 'JS',
  pushed_at: '2026-06-01T00:00:00.000Z', checkedAgeDays: 0, dueInDays: 7,
  needsCheckToday: false, boardOffset: 0, tags: [],
};
const column = { key: 'day-0', title: 'Today', daysAgoTarget: 7, accent: 'rose' };
...
describe('RepoCard long-press (mobile)', () => {
  ...
  it('opens the move sheet after the long-press threshold', () => {
    ...
    expect(screen.queryByRole('dialog', { name: /Reschedule/ })).not.toBeInTheDocument();

    act(() => vi.advanceTimersByTime(500));
    expect(screen.getByRole('dialog', { name: 'Reschedule widget' })).toBeInTheDocument();
  });
  ...
  it('applies the move via onSnooze and closes', () => {
    ...
    fireEvent.change(screen.getByLabelText('Mark done for (days)'), { target: { value: '10' } });
